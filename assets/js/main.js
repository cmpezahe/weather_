...
const fetchData = async (inputdata) => {
  ...
  windElement.textContent = `Wind: ${windValue}`;
  console.log(windValue);

  const precipitationElement = document.getElementById("precipitation");
  const precipitationValue = data.current.precip_mm;
  precipitationElement.textContent = `Precipitation: ${precipitationValue}`;
